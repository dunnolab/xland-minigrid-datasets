...
class FlashAliBiCausalSelfAttention(nn.Module):
    ...
    def forward(self,
                ...
                cache_seqlens: Optional[Union[int, torch.Tensor]] = None) -> torch.Tensor:
        B, L, D = x.size()
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.attention = FlashAliBiCausalSelfAttention(
            hidden_dim, num_heads, attention_dropout, normalize_qk=normalize_qk, with_alibi=with_alibi
        )
...
class EmbeddingEncoder(nn.Module):
    ...
    def forward(self, img: torch.Tensor) -> torch.Tensor:
        img_emb = torch.cat([
            self.entity_emb(img[..., 0]),
            self.color_emb(img[..., 1])
        ], dim=-1)
...
class ObservationEncoder(nn.Module):
    def __init__(self,
                 ...
                 features_dim: int = 64) -> None:
        super().__init__()

        self.embeding_dim = embedding_dim
        self.features_dim = features_dim
        self.emb = EmbeddingEncoder(embedding_dim)
        self.encoder = nn.Sequential(
            nn.Conv2d(2 * embedding_dim, 16, (2, 2), padding="valid"),
            nn.ReLU(),
            nn.Conv2d(16, 32, (2, 2), padding="valid"),
            nn.ReLU(),
            nn.Conv2d(32, 64, (2, 2), padding="valid"),
            nn.ReLU(),
            nn.Flatten(),
    ...
    def forward(self, img: torch.Tensor) -> torch.Tensor:
        ...
        img_transformed = self.emb(img.flatten(0, 1))
        out = self.encoder(img_transformed)
